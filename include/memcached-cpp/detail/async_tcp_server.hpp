...
namespace memcachedcpp { namespace detail {
    ...
    public:
        ...
        async_tcp_server(server_iter begin, server_iter end, const std::string& port) : promises(std::distance(begin,end)) {
    ...
    private:
        ...
        void handle_read_data(std::size_t server_id, std::size_t data_size, const boost::system::error_code&, std::size_t) {
            ...
            boost::asio::async_read_until(sockets[server_id], read_buffer, endmarker(),
        ...
        void async_read_n() {
            for(auto&& server_id : boost::irange<std::size_t>(0, sockets.size())) {
                using namespace std::placeholders;
                boost::asio::async_read_until(sockets[server_id], read_buffer, linefeed(), 
